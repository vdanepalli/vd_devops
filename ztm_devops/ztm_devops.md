# DevOps : Linux, Linux Sysadmin

- [DevOps : Linux, Linux Sysadmin](#devops--linux-linux-sysadmin)
  - [Introduction](#introduction)
  - [Linux Terminal](#linux-terminal)
  - [Linux File System](#linux-file-system)
  - [User Accounts Management](#user-accounts-management)
  - [Linux File Permissions](#linux-file-permissions)
  - [Linux Process Management](#linux-process-management)
  - [Networking in Linux](#networking-in-linux)
  - [Software Management](#software-management)
  - [System Administration](#system-administration)
  - [Bash Shell Scripting](#bash-shell-scripting)
  - [Containers using Podman](#containers-using-podman)
  - [Project 1: Containers using Docker](#project-1-containers-using-docker)
  - [Project 2: Securing and Hardening a Linux System](#project-2-securing-and-hardening-a-linux-system)
  - [Project 3: Securing Internet Traffic with VPNs - WireGuard VPN](#project-3-securing-internet-traffic-with-vpns---wireguard-vpn)
  - [Project 4: Setting up a Web and DNS Server](#project-4-setting-up-a-web-and-dns-server)
  - [Project 5: Automating Linux Administrative Tasks With Ansible](#project-5-automating-linux-administrative-tasks-with-ansible)
  - [IPFS: InterPlanetary File System](#ipfs-interplanetary-file-system)
  - [SSH Public Key Authentication](#ssh-public-key-authentication)
  - [Conclusion](#conclusion)

## Introduction


## Linux Terminal


## Linux File System


## User Accounts Management


## Linux File Permissions


## Linux Process Management


## Networking in Linux


## Software Management 


## System Administration


## Bash Shell Scripting


## Containers using Podman


## Project 1: Containers using Docker


## Project 2: Securing and Hardening a Linux System


## Project 3: Securing Internet Traffic with VPNs - WireGuard VPN


## Project 4: Setting up a Web and DNS Server


## Project 5: Automating Linux Administrative Tasks With Ansible


## IPFS: InterPlanetary File System


## SSH Public Key Authentication


## Conclusion
